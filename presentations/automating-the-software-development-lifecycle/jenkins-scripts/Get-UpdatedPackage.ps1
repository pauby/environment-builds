...
$localPkgs = choco.exe list --source $LocalRepo | Select-Object -Skip 1 | Select-Object -SkipLast 1 | ConvertTo-ChocoObject
...
$localPkgs | ForEach-Object {
    ...
    $remotePkg = choco.exe list $_.name --source $RemoteRepo --exact | Select-Object -Skip 1 | Select-Object -SkipLast 1 | ConvertTo-ChocoObject
if ([version]($remotePkg.version) -gt ([version]$_.version)) {
    ...
    choco.exe download $_.name --no-progress --internalize --force --internalize-all-urls --append-use-original-location --output-directory=$tempPath --source=$RemoteRepo
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        (Get-Item -Path (Join-Path -Path $tempPath -ChildPath "*.nupkg")).fullname | ForEach-Object {
            choco.exe push $_ --source $LocalRepo --api-key $LocalRepoApiKey --force
            if ($LASTEXITCODE -eq 0) {
