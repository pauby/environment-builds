  ...
  provisioner 'windows_reboot' do
    ...
    class RebootProvisioner < Vagrant.plugin('2', :provisioner)
      ...
      def provision
        ...
        prov = Vagrant.plugin(2, :provider)
        sleep 5
        @machine.ui.info("Waiting for the machine to report its IP address...")
        @machine.ui.info("Timeout: 300 seconds")
        counter = 0
        begin
           @machine.ui.info("Counter #{counter}")
          sleep 5
          counter += 5
        end until @machine.communicate.ready? || counter >= 300
